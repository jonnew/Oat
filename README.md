...
which operate on instances of the above datatypes, called `tokens`.  Token
...
- __`camera-model`__=`+int` Value, 0 or 1, specifying the camera model to use.
  0 specifies a Pinhole camera model, 1 specifies fisheye. Generate with
  [oat-calibrate](#calibrate).
- __`camera-matrix`__=
  `[+float,+float,+float,+float,+float,+float,float,+float,+float]`,
  [Camera matrix](https://en.wikipedia.org/wiki/Camera_matrix) for your
  imaging setup. Use [oat-calibrate](#calibrate) to generate.
- __`distortion-coeffs`__=
  `[+float,+float,+float,+float,+float, ...]`, Five to eight element vector
  specifiying lens distortion coefficients. Generated by
  [oat-calibrate](#calibrate).
