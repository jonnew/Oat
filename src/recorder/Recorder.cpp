...
// TODO: Way too much going on in this constructor. Delegate to methods
// or visitors.
...
, sources_eof(false) {
    ...
    if (!frame_source_addresses.empty()) {
        ...
        for (auto &addr : frame_source_addresses) {
            ...
            if (!checkFilePermissions(frame_fid))
                 throw (std::runtime_error("Error: unable to open" + frame_fid +
                        " for writing.\n"));
...
bool Recorder::checkFilePermissions(std::string& file) {
