...
int HomographyGenerator::addDataPoint() {
    ...
    try {
        ...
        if (std::find(pixels_.begin(), pixels_.end(), static_cast<cv::Point2f>(mouse_pt_)) != pixels_.end()) {
        ...
        // Instruct
        std::cout << "Enter <x y> world coordinate followed by <enter> or <return>: ";
        ...
        if (std::find(world_points_.begin(), world_points_.end(), cv::Point2f(dst_pt[0], dst_pt[1])) != world_points_.end()) {
...
int HomographyGenerator::generateHomography() {
    ...
    if (!homography_.empty()) {
        ...
        std::cerr << oat::Error("Homography generation failed.\n")
                  << oat::Error("Check the sanity of your data set and/or try a different transform estimation method.\n");
        return -1;
    }
}
...
int HomographyGenerator::saveHomography() {
    ...
    std::vector<double> temp{homography_.begin<double>(), homography_.end<double>()};
    //cpptoml::array homography{temp.begin(), temp.end()};



    // Add the homography array to the table

    // Add save date-time to the table

    // Save the table as toml file
    //std::ofstream fs;
    //fs.open(calibration_save_path_);

    // TODO: cpptoml to array
    //cpptoml::array homography(homography_.datastart, homography_.dataend);
    //homography.


    //cpptoml::table base;
    //base->insert("homography")
