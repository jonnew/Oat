...
void ArucoBoard::appendOptions(po::options_description &opts)
{
    ...
    local_opts.add_options()
        ("dictionary,D", po::value<std::string>(),
         ...
         "number of markers in the dictionary. Each is encoded by a string "
         "specifying these parameters as <Size>X<Size>_<Number of Markers>. Valid "
         "arguements are:\n"
         "    4X4_50\n"
         "    4X4_100\n"
         "    4X4_250\n"
...
void ArucoBoard::configure(const po::variables_map &vm)
{
    ...
    if (oat::config::getArray<int, 2>(vm, config_table, "board-size", n)) {
        ...
        if (n[0] * n[1] > dict->bytesList.rows) // bytesList is dict size
