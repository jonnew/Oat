...
{
    Visualization::createVisualizationImage(image,
                                            predicted_pose_,
                                            camera_matrix_K_,
...
void PoseEstimator::setMarkerPositions(
    List4DPoints positions_of_markers_on_object)
{
    ...
    histogram_threshold_
        = Combinations::numCombinations(object_points_.size(), 3);
}
...
bool PoseEstimator::estimateBodyPose(cv::Mat image, double time_to_predict)
{
        ...
        < 1) // If not yet initialised, search the whole image for the points
    {
        ...
        LEDDetector::findLeds(image,
                              region_of_interest_,
                              detection_threshold_value_,
                              gaussian_sigma_,
                              min_blob_area_,
                              max_blob_area_,
                              max_width_height_distortion_,
                              max_circular_distortion_,
        ...
        {
            std::cout << "Initialising using brute-force correspondence search."
                      << std::endl;
        ...
        do {
                ...
                >= min_num_leds_detected_) // If enough LEDs detected
            {
                ...
                if (num_loops
                    < 2) { // If haven't searched image yet, search image

                    std::cout << "Too few LEDs detected in ROI. Searching "
                                 "whole image. "
                              << std::endl;
...
void PoseEstimator::predictPose(double time_to_predict)
{
    ...
    // extrapolate
    Vector6d delta_hat = delta / (current_time_ - previous_time_)
                         * (predicted_time_ - current_time_);
...
inline Eigen::Vector2d PoseEstimator::project2d(Eigen::Vector4d point,
...
void PoseEstimator::predictMarkerPositionsInImage()
{
    for (unsigned i = 0; i < object_points_.size(); ++i) {
        predicted_pixel_positions_(i)
            = project2d((Eigen::Vector4d)object_points_(i), predicted_pose_);
    }
}
...
void PoseEstimator::calculateImageVectors()
{
    ...
    for (unsigned i = 0; i < num_image_points; ++i) {
        ...
        single_vector(1)
            = (image_points_(i)(1) - camera_matrix_K_.at<double>(1, 2))
              / camera_matrix_K_.at<double>(1, 1);
        single_vector(2) = 1;
        image_vectors_(i) = single_vector / single_vector.norm();
    }
}

double PoseEstimator::calculateSquaredReprojectionErrorAndCertainty(
    const List2DPoints &image_pts,
    const List2DPoints &object_pts,
    double &certainty)
{
    ...
    for (unsigned i = 0; i < image_pts.size(); ++i) {
        for (unsigned j = 0; j < object_pts.size(); ++j) {
            distances(i, j) = squareDist((Eigen::Vector2d)image_pts(i),
                                         (Eigen::Vector2d)object_pts(j));
        }
    }
    ...
    for (unsigned j = 1; j <= std::min(image_pts.size(), object_pts.size());
         ++j) {
...
void PoseEstimator::findCorrespondences()
{
    ...
    VectorXuPairs pairs = calculateMinDistancesAndPairs(
        predicted_pixel_positions_, image_points_, min_distances);
...
unsigned PoseEstimator::checkCorrespondences()
{
    ...
    if (correspondences_.rows() < 4) {
        ...
        MatrixXYu combinations = Combinations::combinationsNoReplacement(
            correspondences_.rows(), 3);
        unsigned N = combinations.rows();

        for (unsigned i = 0; i < N; ++i) {
            // Declare and populate the feature vectors and world points
            // required for the P3P algorithm
            Eigen::Matrix3d feature_vectors, world_points;
            Eigen::Matrix<Eigen::Matrix<double, 3, 4>, 4, 1> solutions;

            Eigen::Vector4d temp_point;
            temp_point = object_points_(
                correspondences_(combinations(i, 0) - 1, 0) - 1);
            world_points.col(0) = temp_point.head<3>();
            temp_point = object_points_(
                correspondences_(combinations(i, 1) - 1, 0) - 1);
            world_points.col(1) = temp_point.head<3>();
            temp_point = object_points_(
                correspondences_(combinations(i, 2) - 1, 0) - 1);
            world_points.col(2) = temp_point.head<3>();

            feature_vectors.col(0) = image_vectors_(
                correspondences_(combinations(i, 0) - 1, 1) - 1);
            feature_vectors.col(1) = image_vectors_(
                correspondences_(combinations(i, 1) - 1, 1) - 1);
            feature_vectors.col(2) = image_vectors_(
                correspondences_(combinations(i, 2) - 1, 1) - 1);

            // Find the unused image and object points
            unsigned total_unused_correspondences = correspondences_.rows() - 3;
            List2DPoints unused_im_points(
                total_unused_correspondences); // Vector to hold the indexes of
                                               // the unused image points that
                                               // have correspondences
            List4DPoints unused_object_points(
                total_unused_correspondences); // Vector to hold the indexes of
                                               // the unused object points that
                                               // have correspondences
            ...
            for (unsigned l = 0; l < correspondences_.rows(); ++l) {
                ...
                if (!already_used_correspondence) {
                    unused_object_points(num_unused_points)
                        = object_points_(correspondences_(l, 0) - 1);
                    unused_im_points(num_unused_points)
                        = image_points_(correspondences_(l, 1) - 1);
                    num_unused_points++;
                }
            ...
            // If the P3P algorithm found a solution (i.e. the world points were
            // not aligned), then continue
            if (executed_correctly == 0) {
                ...
                for (unsigned j = 0; j < 4; ++j) {
                    ...
                    // Consider only the real poses. Complex or NaN poses are
                    // excluded/ignored
                    if (isFinite(H_o_c_current)) {
                        // Back-project the unused object points onto the image
                        // plane
                        List2DPoints unused_back_projected_object_points(
                            total_unused_correspondences);
                        Eigen::Vector3d temp;
                        for (unsigned ii = 0; ii < total_unused_correspondences;
                             ++ii) {
                            unused_back_projected_object_points(ii)
                                = project2d(unused_object_points(ii),
                                            H_o_c_current.inverse());
                        }

                        double certainty;
                        squared_error
                            = calculateSquaredReprojectionErrorAndCertainty(
                                unused_im_points,
                ...
                if (valid_correspondence_found) {
                    ...
                    temp_solution.block<3, 4>(0, 0)
                        = solutions(smallest_error_idx);

                    for (unsigned jj = 0; jj < object_points_.size(); ++jj) {
                        mean_reprojected_object_points.col(jj)
                            = mean_reprojected_object_points.col(jj)
        ...
        if ((double)num_valid_correspondences / N
            >= valid_correspondence_threshold_) {
            valid_correspondences = 1;
            mean_reprojected_object_points
                = mean_reprojected_object_points / num_valid_correspondences;
            ...
            object_points_matrix.conservativeResize(
                3, object_points_matrix.cols());
            mean_reprojected_object_points.conservativeResize(
                3, mean_reprojected_object_points.cols());
            predicted_pose_ = computeTransformation(
                object_points_matrix, mean_reprojected_object_points);
        }
    }
...
unsigned PoseEstimator::initialise()
{
    ...
    seen_points_working_vector.setLinSpaced(
        image_points_.size(), 1, image_points_.size());
    MatrixXYu seen_points_combinations
        = Combinations::combinationsNoReplacement(seen_points_working_vector,
                                                  3);
    ...
    for (unsigned i = 0; i < num_seen_points_combinations; ++i) {
        ...
        feature_vectors.col(0)
            = image_vectors_(seen_points_combinations(i, 0) - 1);
        feature_vectors.col(1)
            = image_vectors_(seen_points_combinations(i, 1) - 1);
        feature_vectors.col(2)
            = image_vectors_(seen_points_combinations(i, 2) - 1);

        // Find unused image points
        unsigned total_unused_im_points = image_points_.size() - 3;
        List2DPoints unused_im_points(
            total_unused_im_points); // Vector to hold the indexes of the unused
                                     // image points that have correspondences
        ...
        for (unsigned j = 0; j < num_object_points_permutations; ++j) {
            ...
            if (executed_correctly == 0) {
                ...
                List4DPoints unused_object_points(
                    total_unused_object_points); // Vector to hold the indexes
                                                 // of the unused object points
                ...
                for (unsigned ll = 0; ll < object_points_.size(); ++ll) {
                    ...
                    if (!already_used_object_point) {
                        unused_object_points(num_unused_object_points)
                            = object_points_(ll);
                ...
                for (unsigned k = 0; k < 4; ++k) {
                    // Loop through the four solutions provided by the P3P
                    // algorithm
                    H_o_c_current.setIdentity();
                    ...
                    if (isFinite(H_o_c_current)) {
                        ...
                        // plane
                        List2DPoints unused_back_projected_object_points(
                            total_unused_object_points);
                        Eigen::Vector3d temp;
                        for (unsigned m = 0; m < total_unused_object_points;
                             ++m) {
                            unused_back_projected_object_points(m)
                                = project2d(unused_object_points(m),
                                            H_o_c_current.inverse());
                        }

                        Eigen::VectorXd min_distances;
                        MatrixXYu pairs = calculateMinDistancesAndPairs(
                            unused_im_points,
                            unused_back_projected_object_points,
                            min_distances);

                        // Check that at least one of the points was within the
                        // threshold
                        unsigned count_within_pixel_threshold = 0;
                        for (unsigned ll = 0; ll < min_distances.size(); ++ll) {
                            if (min_distances(ll)
                                < back_projection_pixel_tolerance_) {
                        ...
                        if (count_within_pixel_threshold > 0) {
                            ...
                            for (unsigned mm = 0; mm < 3; ++mm) {
                                im_idx = seen_points_combinations(i, mm)
                                         - 1; // image point index
                                obj_idx = object_points_permutations(j, mm)
                                          - 1; // object point index
                                hist_corr(im_idx, obj_idx)
                                    = hist_corr(im_idx, obj_idx) + 1;
                            }

                            for (unsigned nn = 0; nn < min_distances.size();
                                 ++nn) {
                                if (min_distances(nn)
                                    < back_projection_pixel_tolerance_) {
                                    im_idx = unused_im_points_idx(
                                        pairs(nn, 0) - 1); // image point index
                                    obj_idx = unused_object_points_idx(
                                        pairs(nn, 1) - 1); // object point index
                                    hist_corr(im_idx, obj_idx)
                                        = hist_corr(im_idx, obj_idx) + 1;
                                }
                            }
...
void PoseEstimator::optimisePose()
{
    ...
    for (unsigned i = 0; i < max_itr; ++i) {
        ...
        for (unsigned j = 0; j < correspondences_.rows(); ++j) {
            ...
            Eigen::Vector2d p_image_plane = project2d(
                (Eigen::Vector4d)object_points_(correspondences_(j, 0) - 1),
                predicted_pose_);

            // Calculate the error
            Eigen::Vector2d e
                = image_points_(correspondences_(j, 1) - 1) - p_image_plane;
...
void PoseEstimator::predictWithROI(double &time_to_predict,
                                   const cv::Mat &image)
{
    if (it_since_initialized_ >= 2) { // Predict the pose if initialised. If not
                                      // the pose will remain the same as the
                                      // previous step (constant velocity model)
        predictPose(time_to_predict);
    } else { // If not yet fully initialised (Only one pose calculated so far)
        setPredictedTime(time_to_predict);
    }
    ...
    region_of_interest_
        = LEDDetector::determineROI(getPredictedPixelPositions(),
                                    image.size(),
                                    roi_border_thickness_,
                                    camera_matrix_K_,
                                    camera_distortion_coeffs_);
}
...
void PoseEstimator::findCorrespondencesAndPredictPose(double &time_to_predict)
{
    ...
    if (checkCorrespondences()
        == 1) { // If the correspondences were correct, update the pose
        optimiseAndUpdatePose(time_to_predict);
    } else { // Reinitialise if the correspondences weren't correct

        if (initialise() == 1) { // Only update pose if the initialisation found
                                 // a valid pose.
            optimiseAndUpdatePose(time_to_predict);
        }
    }
...
double PoseEstimator::squareDist(const Eigen::MatrixBase<DerivedA> &p1,
...
VectorXuPairs
PoseEstimator::calculateMinDistancesAndPairs(const List2DPoints &points_a,
                                             const List2DPoints &points_b,
                                             Eigen::VectorXd &min_distances)
{
    ...
    // Work around since pairs.col(0).setLinSpaced(num_points_a,1,num_points_a)
    // throws a floating point error when num_points_a = 1
    if (num_points_a == 1) {
        pairs(0) = 1;
    } else {
    ...
    for (unsigned i = 0; i < num_points_a; ++i) {
        ...
        for (unsigned j = 0; j < num_points_b; ++j) {
            dist_squared = squareDist((Eigen::Vector2d)points_a(i),
                                      (Eigen::Vector2d)points_b(j));
...
Eigen::Matrix4d
PoseEstimator::computeTransformation(const MatrixXYd &object_points,
                                     const MatrixXYd &reprojected_points)
{
    Eigen::Vector3d mean_object_points
        = object_points.rowwise().sum() / object_points.cols();
    Eigen::Vector3d mean_reprojected_points
        = reprojected_points.rowwise().sum() / reprojected_points.cols();
    MatrixXYd object_points_bar
        = object_points.colwise()
          - mean_object_points; // object points with zero mean
    MatrixXYd reprojected_points_bar
        = reprojected_points.colwise() - mean_reprojected_points;

    Eigen::JacobiSVD<MatrixXYd> svd_of_points(
        object_points_bar * reprojected_points_bar.transpose(),
...
Matrix2x6d PoseEstimator::computeJacobian(const Eigen::Matrix4d &T_c_o,
                                          const Eigen::Vector4d &world_points,
                                          const Eigen::Vector2d &focal_lengths)
{
    // This Jacobian is calculated according to equation A.14 from the the PhD
    // thesis of Ethan Eade
...
Eigen::Matrix4d PoseEstimator::exponentialMap(const Vector6d &twist)
{
    ...
    if (theta == 0) {
        ...
        V = (Eigen::Matrix3d::Identity()
             + (1 - cos(theta)) / (theta_squared)*Omega
...
Vector6d PoseEstimator::logarithmMap(const Eigen::Matrix4d &trans)
{
    ...
    if (R.isApprox(Eigen::Matrix3d::Identity(), 1e-10) == 1) {
        ...
        // Force phi to be either 1 or -1 if necessary. Floating point errors
        // can cause problems resulting in this not happening
        if (temp > 1) {
            temp = 1;
        } else if (temp < -1) {
            temp = -1;
    ...
    if (t.isApproxToConstant(0, 1e-10) == 1) {
        ...
        A_inv = Eigen::Matrix3d::Identity() - w_hat / 2
                + (2 * sin(w_norm) - w_norm * (1 + cos(w_norm)))
                      / (2 * w_norm * w_norm * sin(w_norm)) * w_hat * w_hat;
    }
