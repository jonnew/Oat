...
void LEDDetector::findLeds(
    ...
    const double &min_blob_area,
    const double &max_blob_area,
    const double &max_width_height_distortion,
    const double &max_circular_distortion,
    ...
    const std::vector<double> &camera_distortion_coeffs)
{
    ...
    cv::threshold(
        image(ROI), bw_image, threshold_value, 255, cv::THRESH_TOZERO);
    ...
    cv::findContours(gaussian_image.clone(),
                     contours,
                     CV_RETR_EXTERNAL,
    ...
    if (numPoints > 0) {
        ...
        cv::undistortPoints(distorted_points,
                            undistorted_points,
                            camera_matrix_K,
...
cv::Rect
LEDDetector::determineROI(List2DPoints pixel_positions,
                          cv::Size image_size,
                          ...
                          const std::vector<double> &camera_distortion_coeffs)
{
    ...
    // Distort the points
    distortPoints(undistorted_points,
                  distorted_points,
                  camera_matrix_K,
                  camera_distortion_coeffs);
